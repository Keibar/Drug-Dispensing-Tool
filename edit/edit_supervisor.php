...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Retrieve updated supervisor details from the form
    $name = $_POST['name'];
    $pharmaceuticalId = $_POST['pharmaceuticalId'];
    $emailAddress = $_POST['emailAddress'];
    $SSN = $_POST['SSN'];

    // Update supervisor details in the database
    $conn = new mysqli($host, $username, $databasePassword, $databaseName);
    if ($conn->connect_error) {
        die("Connection failed: " . $conn->connect_error);
    } else {
        ...
        if ($conn->query($sql) === TRUE) {
            // Redirect to supervisor profile page after successful update
    ...
    <div class="container mt-5">
        ...
        <form method="post">
                ...
                <select class="form-control" id="pharmaceuticalId" name="pharmaceuticalId" required>
                    ...
                    if ($conn->connect_error) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            $selected = $supervisor['pharmaceuticalId'] === $row['pharmaceuticalId'] ? 'selected' : '';
            ...
            <div class="mb-3">
                ...
                <input type="email" class="form-control" id="emailAddress" name="emailAddress" value="<?= $supervisor['emailAddress'] ?>" required>
            </div>
            <div class="mb-3">
                <label for="SSN" class="form-label">SSN</label>
                <input type="text" class="form-control" id="SSN" name="SSN" value="<?= $supervisor['SSN'] ?>" required>
            </div>
